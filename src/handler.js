...
const registerHandler = async (request, h) => {
    ...
    const newuser = {
        user_id, namalengkap, nomortelp, email, password 
    }
...
const addTukang = async (request, h) =>{
    ...
    const newtukang = {tukang_id, namatukang, spesialis, review, booked};
...
const searchHandler = async (request, h) => {
    ...
    try {
        ...
        let tukangSnapshot;
        if (namatukang){
            tukangSnapshot = await tukangRef.where('namatukang', '>=', namatukang).where('namatukang', '<=', namatukang + '\uf8ff').get();
